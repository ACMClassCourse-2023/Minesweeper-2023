...
首先，我们将向服务端代码输入游戏地图，并为你进行第一步操作（从而简化游戏，防止第一步就碰到地雷）。接下来，用户端代码可以多次提交操作指令，并读取服务端的输出。
...
注意这里的最后一行是游戏结束（失败）的输出，`step_count` 为 5 是因为将第一次操作 `visit 2 0` 计算入中操作数。
