...
随后，你会通过标准输入得到若干行字符串来表示玩家的操作，每一行有两个正整数 `x y`，分别是两个在 `[0, n - 1]` 和 `[0, m - 1]` 范围的数字，代表玩家这一步希望访问第 `x` 行第 `y` 列的格子。
...
- 输出地图后，你需要判定游戏是否结束（失败或胜利），若游戏结束，你应当先输出一行文字，**胜利则为 "YOU WIN!\n"，失败则为 "GAME OVER!\n"**，再输出一行两个整数 `visit_count step_count`，以空格分隔，分别代表玩家访问过的**非地雷格子**的个数与总操作数，并立即以 0 为返回值退出程序。
   - 例如，在上面的例子中，当你输出完地图后，你应输出下面的信息并退出程序：
      ```
      GAME OVER!
      4 3
      ```
